...
```json5
{
  ...
  "description": "This is a example.disable plugins",
  "version": "1.0.0",
  "author": "kenftr",
  "license": "MIT",
  "open_source": true,
...
- `init_variable:` Các tham số cần thiết cho hàm, Swit sẽ tự động inject khi load.
...
async def on_start(app):
    ...
    print(latency)
```
> Trong ví dụ trên, hàm `on_start` cần tham số `app`. Vì vậy, ta đưa app vào init_variable kèm với api bạn cần ở đây ta sử dụng `Swit.app`.<br>

`Swit.app` là object commands.Bot của discord.py
...
`format`: format của mapping. Hiện tại là `Swit-mapping-v2`<br>
...
```
`slash_command_path` = `slash_command` thì Swit sẽ load các **slash command** tại `example/slash_command`
...
```
với file name là file chứa class chính. Còn class là nơi mà Swit sẽ import.
