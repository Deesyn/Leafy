...
- `init_variable:` Các tham số cần thiết cho hàm, Leafy sẽ tự động inject khi load.
...
async def on_start(app):
    ...
    print(latency)
```
> Trong ví dụ trên, hàm `on_start` cần tham số `app`. Vì vậy, ta đưa app vào init_variable kèm với api bạn cần ở đây ta sử dụng `Leaf.app`.<br>

`Leaf.app` là object commands.Bot của discord.py
...
`format`: format của mapping. Hiện tại là `Leafy-mapping-v2`<br>
...
```
`slash_command_path` = `slash_command` thì Leafy sẽ load các **slash command** tại `example/slash_command`
...
```
với file name là file chứa class chính. Còn class là nơi mà Leafy sẽ import.
